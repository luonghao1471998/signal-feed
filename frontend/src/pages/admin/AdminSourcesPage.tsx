          ...
          if (action === "approve") {
            return (
              <Button
                ...
                className="bg-gradient-to-r from-slate-600 to-slate-600 text-white shadow-sm shadow-slate-600/20 hover:from-slate-500 hover:to-slate-500"
              >
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-slate-900">Source Moderation Queue</h1>
          <p className="text-sm text-slate-600">Review and moderate user-submitted sources.</p>
        </div>
      ...
      {!isLoading && !isError ? (
        <div className="overflow-x-auto rounded-lg border border-zinc-200 bg-white shadow-sm">
          <p className="px-4 pt-3 text-sm text-slate-600">
            {meta ? formatAdminTableRange(meta.current_page, meta.per_page, meta.total) : ""}
          </p>
          <Table>
            <TableHeader>
              <TableRow>
              ...
              {filteredSources.length === 0 ? (
                ...
                filteredSources.map((source, index) => (
                  <TableRow key={source.id}>
                    <TableCell>{(page - 1) * (meta?.per_page ?? 20) + index + 1}</TableCell>
                    <TableCell>
                      <a
                        href={source.account_url ?? `https://x.com/${source.x_handle}`}
                        target="_blank"
                        rel="noreferrer"
                        className="font-medium text-slate-700 underline decoration-slate-200 underline-offset-2 hover:text-slate-600"
                      >
                        @{source.x_handle}
                      </a>
                    </TableCell>
                        ...
                        {source.categories.length > 0
                          ? source.categories.map((category) => (
                              <span
                                ...
                                className="rounded-full bg-slate-50 px-2 py-0.5 text-xs text-slate-800 ring-1 ring-slate-100/80"
                              >
                                {category.name}
                              </span>
                            ))
