...
**Đồng bộ roadmap (2026-04-06):** `IMPLEMENTATION-ROADMAP.md` và file này khớp review: crawl **4×/ngày**; twitterapi **`advanced_search`** theo **Báo cáo POC**; `sources.last_crawled_at` + vòng lặp crawl; **`TweetFetchProviderInterface`**; clustering **prompt-based**; `users.is_admin`; bảng **`user_personal_feed_entries`** (Sprint 2+). Hợp đồng vendor chi tiết = **`SPEC-api.md`** (canonical); `API-CONTRACTS.md` là bản tóm tắt đồng bộ.
...
- **Phase 1 — Option A:** user thêm nguồn → `status='active'` ngay; admin **hậu kiểm** (Flow 6), không happy-path `pending_review`
...
**Cap My KOLs (SPEC Mục 7 Flow 2):** Pro ≤10, Power ≤50 subscription — kiểm tra **trước** khi tạo `MySourceSubscription`; vượt → 403 + upgrade.
...
**Free tier (SPEC Mục 6):** 3 digest/tuần (Mon/Wed/Fri — enforcement tại job/API theo spec); không My KOLs; không draft copy; không email/Telegram (web only).
...
**Trong Phase 1 (tóm tắt):** OAuth, pool KOL, pipeline AI, digest web, My KOLs (Pro/Power), draft + Twitter intent, tier Free/Pro/Power, Stripe, admin moderation + pipeline monitor, SPA responsive.

**Ngoài / hoãn:** Auth email/password chính, Telegram real-time, email digest (tuỳ sprint — xem OQ), full-text archive, “Flag as Noise” (OQ), app native, đa ngôn ngữ UI.

---

## Wedge & Kill Checkpoint

**Sprint 1 (wedge):** Theo **SPEC-core Flow 3**: crawl (scheduler **4×/ngày**) → classify → cluster → summarize → rank → draft; UI digest; attribution; draft. **Tích hợp crawl:** inject **`TweetFetchProviderInterface`**; implementation đúng **POC**; cập nhật **`last_crawled_at`**; loop/stagger theo rate limit — task **1.6.x** trong `IMPLEMENTATION-ROADMAP.md`.
...
- **Source workflow Phase 1:** Option A — `type=user` → `active` ngay; moderation sau (đã chốt 2026-04-03, đồng bộ SPEC + roadmap task 3.3.x).
