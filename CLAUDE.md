...
**Đồng bộ roadmap (2026-04-06 + CR 2026-04-15):** `IMPLEMENTATION-ROADMAP.md` và file này khớp review: crawl **4×/ngày**; twitterapi **`advanced_search`** theo **Báo cáo POC**; `sources.last_crawled_at` + vòng lặp crawl; **`TweetFetchProviderInterface`**; clustering **prompt-based**; `users.is_admin`; onboarding Step 2 `/onboarding/sources` (filter `my_categories`, follow/skip); bảng **`user_personal_feed_entries`** (Sprint 2+). Hợp đồng vendor chi tiết = **`SPEC-api.md`** (canonical); `API-CONTRACTS.md` là bản tóm tắt đồng bộ.
...
**Trong Phase 1 (tóm tắt):** OAuth, onboarding 2-step (category + optional KOL follow/skip), pool KOL, pipeline AI, digest web, My KOLs (Pro/Power), draft + Twitter intent, tier Free/Pro/Power, Stripe, admin moderation + pipeline monitor, SPA responsive, **Archive save/list flow**, **Settings persistence**, **đa ngôn ngữ UI (en/vi baseline)** theo roadmap cập nhật.
...
| **Personal feed (Pro/Power)** | **Đã lock schema/API** (`user_personal_feed_entries`); triển khai job/UI **Sprint 2+**, ngoài **69 task** roadmap |
